...
`GRanges` objects can be used as the input regions to all of iscream's querying
functions. `tabix_gr()` and `make_mat_gr()` return `GRanges` objects. The
outputs of of `tabix()` and `make_mat()`, `make_bsseq_mat()` can also be turned
into `GRanges`.
...
`summarize_regions` returns a data frame with a feature column identifying each
summary row's genomic region.

If the regions don't have names (see `?summarize_regions`), pass the `feature`
column with the genomic regions as the GRanges input regions:
