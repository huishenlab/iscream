...
parsed `r CRANpkg("data.table")`. To get a `GRanges` object instead, use
`tabix_gr`.
...
files, but it only supports one file at a time and only accepts `GRanges` as the
input regions. See `vignette("tabix")` for a comparison of iscream and
Rsamtools.
...
`tabix()` accepts `GRanges` objects as input regions. To get `r Biocpkg("GenomicRanges")`
objects instead of data frames, use `tabix_gr` which will also preserve any
input `GRanges` metadata.
...
coordinates, and file names you can use to make other data structures.
`make_mat_se()` returns a `r Biocpkg("RangedSummarizedExperiment")`, and
`make_mat_gr()` returns a `GRanges` object.
