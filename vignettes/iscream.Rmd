...
`tabix_gr`. As with tabix, any duplicated records, either in the file or because
of overlapping input regions, will be returned as-is. To get only the unique
records, use `unique()`.
...
> **Note:** because the tabix API does not currently support multiple regions as
> input, it retrieves duplicate records if the input regions overlap and contain
> overlapping records. With `tabix()` you can use `unique()` to filter these
> out.
...
> **Note:** as with `tabix()`, you may have duplicated records if your input
regions overlap. Check the `mat$chr` and `mat$pos` vectors to find duplicate
loci. With `make_mat_gr()` and `make_mat_se()` outputs, you can find duplicate
loci with `GRanges`.
