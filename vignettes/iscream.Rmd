...
multiple [tabixed](https://en.wikipedia.org/wiki/Tabix) BED files based on
...
of interest. It queries and prints the loci from the tabixed BED file that fall
within the regions of interest. This output may be redirected to a file and read
into R.
...
columns or a `GRanges` object. It queries the BED files and returns the loci as
a parsed `r CRANpkg("data.table")`. To get a `GRanges` object instead, use
`tabix_gr`.
...
Using the same bedfiles and regions, iscream can also summarize the data within
