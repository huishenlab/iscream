...
make_sparse_mat <- function(cpg_bed_file, sample_list, sample_path, to_mm = TRUE, file_ext = ".bed.gz") {
  ...
  if (to_mm) {
    ...
    fwrite(list("%%MatrixMarket matrix coordinate real general"), "result.mtx", compress = "gzip")
    fwrite(
      list(nrow(cpg_index), length(sample_list), nrow(result)),
       "result.mtx", sep = " ", compress = "gzip", append = TRUE)
    fwrite(result, "result.mtx", sep = " ", append = TRUE, compress = "gzip")
    message("Done")
  } else {
    list(cpg_index = cpg_index, sample_index = sample_list, sparse_mat = result)
  }
